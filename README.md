...
All three parameters are optional and independent: `outputReference` alone determines the checkpoint, and `originHint` / `originAppVersion` (below) can be set or omitted at either checkpoint.
...
**Origin app version** is the version of the app that produced the event, e.g. `{{Event Data - app_version}}`. It is named to pair with Origin hint: Origin hint says *which* source, Origin app version says *that source's* version. Like the other two parameters it is trimmed (numbers and booleans are stringified) and sent as a top-level field alongside the event schema, never inside the event's own properties.

The parameter is called `originAppVersion`; the field it sets on the wire keeps its own name, `appVersion`. That field is not gateway-specific — it has carried the container-level version on ordinary events all along, and this parameter only overrides it per event.
...
| --- | --- | --- |
| set | provided | the Origin app version value |
| set | empty | `null` — no version at all; the container default is deliberately not applied |
| empty | provided | the Origin app version value (overrides the container default) |
| empty | empty | the container default, `unversioned GTM server-side tag` (unchanged behavior) |

"Empty" means an empty string, a whitespace-only string, or a value the tag cannot turn into a string (an object or an array). Unlike `outputReference` and `originHint`, which are omitted entirely when empty, `appVersion` is always present on the body — as a literal JSON `null` in the second row above. Avo records a `null` app version as `unversioned`; the event itself is stored either way.
...
Every Avo Inspector sender — the SDKs and both GTM templates — now posts to this one endpoint and identifies itself with `X-Avo-Client`, so traffic can be attributed at the edge without decoding a body. `X-Avo-Client: gtm-server` is what marks a request as coming from this template. The header and the body's `libPlatform` field name the same sender and must carry the same string, so `libPlatform` is the constant `gtm-server` too.
...
Because the Inspector key now travels in a request header, the tag refuses to send at all if the configured key contains a carriage return, a line feed or a NUL — characters that delimit header fields and could otherwise append headers to the request or split it in two. The tag does not strip or escape them: a silently rewritten key would authenticate as a different key than the one configured. Such a request calls `gtmOnFailure`, and Preview mode logs why (never the key itself).

Four things changed when the tag moved here from the previous server-side GTM endpoint (`/inspector/gtm/v1/track`):
...
- **Attribution changed.** Events used to be recorded with the tracking ID `GTM`, which the previous endpoint stamped on everything it handled. They are now recorded with `writeApiV2`, which this endpoint stamps on everything *it* handles, so the tracking ID no longer identifies server-side GTM. The `X-Avo-Client: gtm-server` header is what distinguishes this traffic instead. The body's `trackingId` field, always empty and now forbidden on this endpoint, is no longer sent.
- **`libPlatform` is now a constant.** It used to be the name of the server-side GTM Client that claimed the incoming request, so it varied per request and named the Client rather than the sender. It is now always `gtm-server`, matching the `X-Avo-Client` header.
- **The 200 body changed shape.** See the next section.

The body still carries an empty `sessionId`. This tag has no session, and the field is on its way out of Avo's wire format — but the ingestion parser still requires it today, and an event that omits it is discarded *after* the endpoint has already answered `200`, with no signal reaching the sender. Do not remove it until ingestion supplies the value itself.
...
| `200 {"ok":false}` | Accepted, then nothing stored: the endpoint could not decode the request body into events. The body carries a decode-failure count, the status stays `200`, and nothing else raises a per-sender signal. An unexpected server-side error answers with the same shape, because the API's error handler responds without setting a status. |
| `400 {"ok":false,"error":"..."}` | Rejected before acceptance: a missing or invalid Inspector key or environment header, or an Inspector key the API does not recognize. |

Only the first three reach the tag as a success — any non-2xx (including the 400 row) calls `gtmOnFailure`.

In GTM Preview mode, the tag inspects the body of a 2xx response and logs a console warning for the two `false` shapes, so an event-limit drop or a decode failure is visible while you are testing rather than looking like a clean success. The warning prints the response body as its second argument, trimmed of surrounding whitespace, rather than just an error message, so whatever else the endpoint said is visible too — it is Avo's own API response and carries no event data. This logging only runs in Preview mode; it does not change whether the tag reports success or failure to GTM (`gtmOnSuccess`/`gtmOnFailure` are driven solely by the HTTP status code, unchanged by this behavior).
